...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='ppclab-upgrade-test-') as td:
  ...
  for dst,ver in ((current,'3.8.0'),(incoming,'3.9.1')):
   ...
   txt=(dst/'CMakeLists.txt').read_text(); txt=txt.replace('VERSION 3.9.1','VERSION '+ver); (dst/'CMakeLists.txt').write_text(txt)
