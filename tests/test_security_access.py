...
with tempfile.TemporaryDirectory(prefix='ppclab-security-') as td0:
 ...
 digest=hashlib.sha256(code.read_bytes()).hexdigest(); rec={'schema':'ppc-lab-fleet-job-result-v1','name':'secure-evidence','source':'test','cache_key':'a'*64,'engine_version':'3.8.0','host':'unit','inputs':{'image.path':{'logical_path':'leaf.bin','size':8,'sha256':digest}},'response':{'schema':'ppc-lab-worker-response-v1','id':'e','ok':True,'exit_code':0,'timed_out':False,'engine_version':'3.8.0','result':{'schema':'ppc-lab-result-v1','backend':'builtin-ppc32be','stop_reason':'return','instructions':2,'pc':'0x10000004','registers':{'r3':'0x0000002a'},'dumps':[]}}}
