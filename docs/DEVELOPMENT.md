...
1. reject unsupported class, endian, machine, type, or relocation behavior explicitly;
2. validate every file and virtual-memory range before mapping;
3. keep BSS/permission behavior deterministic;
4. add synthetic files generated by tests rather than committing proprietary binaries;
5. prove both inspection and execution paths when the loader feeds `CallHarness`;
6. update `docs/ELF32.md` whenever the accepted format contract changes.

Do not accept `ET_DYN`/`ET_REL` merely because a subset happens to map on one sample. Implement and test required relocation semantics first.
